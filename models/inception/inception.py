...
def Net():
    model = Inception3(num_classes = len(os.listdir(os.path.join(os.getcwd(), 'Data', 'Training'))), transform_input = True)
    return model
...
#----------------------------------------------------------------------------------------
    ...
    pretrained_state = model_zoo.load_url('https://download.pytorch.org/models/inception_v3_google-1a9a5a14.pth')
...
#----------------------------------------------------------------------------------------
    ...
    lossPrint = 0.01
    optimizer = optim.SGD(net.parameters(), lr=lossPrint, momentum=0.9, weight_decay=1e-6)
#----------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------
    resFile = 'inception_Results.txt'
    ...
    for epoch in range(9999):  # loop over the dataset multiple times
        ...
        #epoch_loss = 0.0
        epoch += lastEpoch
        torch.save(net.state_dict(), '/workspace/shared/biometrics-project/models/inception_{:04}.ptm'.format(epoch))
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            loss.backward()
            optimizer.step()
            print('{}.{}: Loss = {} (lr = {}, time = {})'.format(epoch+1, i+1, loss.data[0], lossPrint, float(time.time()-currTime)*(2250/3600)))
            currTime = time.time()
